...
plot_hfi_protected_areas <- function(HFI_data, protected_area, provinces, azerbaijan_provinces){
...
label_protected <- grobTree(textGrob("Protected
  Areas", x=0.62,  y=0.80, hjust=0, gp=gpar(col="#ff8080", fontsize=11)))

# 6.) Plot data
region_hfi <- ggplot() +
  geom_spatraster(data = HFI_data_spatrast) +
  #geom_sf(data = provinces, fill = NA, color = "black") +
  geom_sf(data = azerbaijan_provinces, fill = NA, color = "white") +
  geom_spatvector(data = protected_area_spatvect_hfi_projection, alpha = 0.4, color = "#ff8080") +
  hfi_scale +
  theme_void() +
...
# HFI 2018 data
#plot_hfi_protected_areas(hfi_2018_georgia, protected_area_georgia, georgia_provinces, georgia_provinces)
